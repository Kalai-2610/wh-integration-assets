...
module.exports.getAllResources = async (req, res) => {
    try {
        ...
        const total = await MongoDB.resources.countDocuments({ is_active: true });
...
module.exports.getResource = async (req, res) => {
    try {
        const resource = await MongoDB.resources.findOne({ _id: new ObjectId(req.params.id) });
...
module.exports.createResource = async (req, res) => {
    try {
        ...
        const invalid_auth_methods = allowed_auth_methods.filter(auth_method => !valid_auth_methods.includes(auth_method));
        ...
        const existingResource = await MongoDB.resources.find({ $or: [{ api_path }, { name }, { reference_name }] }).toArray();
        ...
        if (existingResource.length > 0) {
            ...
            throw new AppError('API path or name or reference name already in use', 409, { errors: Array.from(errors) });
        }
...
module.exports.updateResource = async (req, res) => {
    try {
        ...
        const existingResource = await MongoDB.resources.find({ $or: [{ api_path }, { name }], _id: { $ne: new ObjectId(resourceId) } }).toArray();
        errors = new Set();
        if (existingResource.length > 0) {
            existingResource.forEach(each => {
                if (each.name === name) {
        ...
        const result = await MongoDB.resources.updateOne({ _id: new ObjectId(resourceId), is_active: true }, { $set: updateData });
...
module.exports.deleteResource = async (req, res) => {
    ...
    try {
        ...
        const result = await MongoDB.resources.updateOne({ _id: new ObjectId(resourceId), is_active: true }, { $set: deleteData });
        if (result.modifiedCount === 0) {
            throw new AppError('Resource not found or no changes made', 404);
        }
