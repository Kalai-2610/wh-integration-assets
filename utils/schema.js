...
const NAME_REGEX = /^[a-zA-Z][a-zA-Z0-9_]{0,20}$/;
...
function buildStringSchema(field, errors) {
	...
	if (field.options) {
		if (!Array.isArray(field.options) || field.options.some((v) => typeof v !== 'string')) {
		...
		forbidden.forEach((prop) => {
			if (field[prop] !== undefined) addError(errors, field.key, `${prop} not allowed when options is defined`);
		});
...
function buildNumberSchema(field, errors) {
	...
	if (field.decimal) {
		...
		if (minPlaces > maxPlaces) addError(errors, field.key, `decimal_min_places cannot exceed decimal_max_places`);
	...
	if (field.min !== undefined) {
		if (field.min < LIMITS.NUMBER_MIN) addError(errors, field.key, `min < ${LIMITS.NUMBER_MIN}`);
		schema = schema.min(field.min);
	}

	if (field.max !== undefined) {
		if (field.max > LIMITS.NUMBER_MAX) addError(errors, field.key, `max > ${LIMITS.NUMBER_MAX}`);
...
function buildDateTimeSchema(field, errors) {
	...
	if (field.min !== undefined) {
		if (ISO_UTC_STRICT.test(field.min)) {
			...
			if (minDateObj < LIMITS.DATE_MIN) addError(errors, field.key, `datetime min < ${LIMITS.DATE_MIN_STR}`);
		} else {
	...
	if (field.max !== undefined) {
		if (ISO_UTC_STRICT.test(field.max)) {
			...
			if (maxDateObj > LIMITS.DATE_MAX) addError(errors, field.key, `datetime max > ${LIMITS.DATE_MAX_STR}`);
		} else {
	...
	schema = schema.custom((value, helpers) => {
		...
		if (Number.isNaN(date.getTime())) return helpers.error('datetime.invalid');

		if (date < LIMITS.DATE_MIN) return helpers.error('datetime.min');
		if (date > LIMITS.DATE_MAX) return helpers.error('datetime.max');

		if (minDateObj && date < minDateObj) return helpers.error('datetime.fieldMin');
		if (maxDateObj && date > maxDateObj) return helpers.error('datetime.fieldMax');
		return value;
	});
	return schema;
}
...
function buildObjectSchema(field, errors) {
	...
	field.keys.map((child) => {
        if (!child.key || typeof child.key !== 'string') {
			addError(errors, `${field.key}.${child.key}`, 'key is required');
		} else if(typeof child.key !== 'string') {
            addError(errors, `${field.key}.${child.key}`, 'key must br string');
        } else if (!NAME_REGEX.test(child.key)) {
            addError(errors, `${field.key}.${child.key}`, `failed to match the pattern ${NAME_REGEX}`)
        } else if (obj[child.key]) {
			addError(errors, `${field.key}.${child.key}`, `Duplicate key found`);
		} else {
...
function buildFieldSchema(field, errors) {
	...
	if (field.is_multiple) {
		...
		schema = Joi.array().items(schema).min(min).max(max);
...
async function generateJoiSchema(metadata) {
	const errors = [];
	const obj = {};
	const system_keys = CacheMechanism.get('system_keys');
	metadata.map((field) => {
		if (!field.key || typeof field.key !== 'string') {
			addError(errors, field.key, 'key is required');
		} else if(typeof field.key !== 'string') {
            addError(errors, field.key, 'key must br string');
		} else if(system_keys.includes(field.key)) {
            addError(errors, field.key, 'Restriced key');
        } else if (!NAME_REGEX.test(field.key)) {
            addError(errors, field.key, `failed to match the pattern ${NAME_REGEX}`)
        } else if (obj[field.key]) {
